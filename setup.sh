...
# Installing diyHue
msg "Installing diyHue..."
MAC_ADDRESS=$(ip a s dev eth0 | awk '/ether/ {print $2}')
docker volume create diyhue_config >/dev/null

docker run -d --name diyHue \
                --restart=always --network=host \
                -e MAC=${MAC_ADDRESS} \
                -v diyhue_config:/opt/hue-emulator/config \
                diyhue/core:latest &>/dev/null
